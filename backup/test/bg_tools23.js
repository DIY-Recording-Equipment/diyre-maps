...
function BuildGuide(svg, tempSteps, saveExists = false) {
  ...
  var activePcb;
  //Buttons
  var pcbButton, sidButton, dimButton, resButton, winButton;
  // dom containers
  ...
  //var barCycle = 0;
  var kitVis = true;
  //localSaver Save handler
  const localSaver = new LocalSaver();

  //------------------------------------------------------SAVEDATA-INITILIZATION
  ...
  var setupPcbState = function () {
    ...
    let savedParts = localStorage.saveData.split(",");

    for (let x in pars) {
      // console.log("check: " + pars[x].id);
      for (let y in savedParts) {

        if (pars[x].id == savedParts[y]) {
          pars[x].changed = "true";
          pars[x].state = StateEnum.clicked;
        }
  ...
  var makeControls = function () {
    ...
    resButton.onclick = resTgl;
    resButton.style.marginTop = "0px";
    cont.appendChild(resButton);
  ...
  var makeList = function () {
    ...
    for (let i = 1; i <= maxStep; i++) {
      ...
      stepHead.innerHTML = "<i>" + steps["T" + i][0] + " </i> " + steps["T" + i][1];
  ...
  var updatePar = function () {
    ...
    var toChange = pars.filter(function (x) {
      if (x.changed == "true") {
        x.changed = "false";
        return x;
      }
  ...
  var findPcb = function () {
    ...
    var activePar = pars.filter(function (x) {
      if (x.state == StateEnum.highlight) {
        return x;
      }
    });
  ...
  var Part = function (domIn) {
    ...
    this.clickPart = function () {
      ...
      */
      ...
      if (this.state > 0) {
        localSaver.addPart(this.id);
      } else {
        localSaver.removePart(this.id);
      }
...
function LocalSaver() {
  ...
  this.hasPart = (id) => {
    ...
    if (localStorage.saveData.indexOf(id)) {
      return false;
    } else {
      return true;
    }
  ...
  this.addPart = (id) => {
    localStorage.saveData += id + ",";
  };

  this.removePart = (id) => {

    let index = localStorage.saveData.indexOf(id);

    console.log(index);

    localStorage.saveData = localStorage.saveData.replace(id + ",", "");
