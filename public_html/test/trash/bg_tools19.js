...
//------------------------------------------------------------FUNCTS
    ...
    //optionalParts.length is subtracted from the total number of parts so that optional parts don't contribute to the 100%age. 
      let selOptionalParts = pars.filter( function(x) { return x.part.attr("optional") == "true" && x.state != StateEnum.unclicked; }).length;
        
      //console.log(pars.length + " -- " + selected);
      // console.log(100 / pars.length * selected);
      // console.log(youWin);
        if((100 / (pars.length - selOptionalParts) * selected) <= 100){
            document.getElementById("myBar").style.width = (100 / (pars.length - selOptionalParts) * selected) + "%";
        };
        console.log(selOptionalParts);
