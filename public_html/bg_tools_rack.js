...
function BuildGuide(svg, tempSteps) {
  ...
  var updatePar = function () {
    ...
    while (toChange.length > 0) { //if this is 0 it erroneously highlights if 1 it doesn't highlight
      tempCh = toChange.pop()
  ...
  function kitClick(x) {
    ...
    composedPath = x.composedPath()
    console.log(composedPath)
    
    for (let i = 0; i < composedPath.length; i++) {
      if(composedPath[i].id != '' && !composedPath[i].id.includes("LWPOLY")) {
        firstG = composedPath[i]
        break;
      }
    }
    ...
    if (firstPart) {
      ...
      console.log("clicked a part")
    } else {
      console.log("didn't click a part")
    }
  }
};
