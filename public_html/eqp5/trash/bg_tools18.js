...
//------------------------------------------------------------FUNCTS
    ...
    //optionalParts.length is subtracted from the total number of parts so that optional parts don't contribute to the 100%age. 
      let optionalParts = pars.filter( function(x) { return x.part.attr("optional") == "true"; }).length;
        
      //console.log(pars.length + " -- " + selected);
      // console.log(100 / pars.length * selected);
      // console.log(youWin);
        if((100 / (pars.length - optionalParts) * selected) <= 100){
            document.getElementById("myBar").style.width = (100 / (pars.length - optionalParts) * selected) + "%";
        };
