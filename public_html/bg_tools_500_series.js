...
function BuildGuide(svg, tempSteps) {
  ...
  var activePcb;
  //Buttons
  var pcbButton, sidButton, dimButton, winButton, deleteSaveButton;
  // dom containers
  ...
  var setupPcbState = function () {
    ...
    //set PCB to mother board
    if (activePcb.node.id == "DB") {
      pcbTgl()
    }
  ...
  var makeList = function () {
    ...
    for (let i = 1; i <= maxStep; i++) {
      ...
      if (Array.isArray(steps["T" + i])) {
        stepHead.innerHTML = "<i>" + steps["T" + i][0] + " </i> " + steps["T" + i][1];
      } else {
        stepHead.innerHTML = "<i>" + steps["T" + i] + " </i>";
      }
  ...
  var updatePar = function () {
    ...
    toChange = pars.filter(function (x) {
      if (x.changed == "true") {
        x.changed = "false";
        return x;
      }
  ...
  var Part = function (domIn) {
    ...
    this.clickPart = function () {
      ...
      if (this.state == 0) {
        ...
        highlighter.part = domPart
        highlighter.box = svg.select("#KIT").rect(
          domPart.getBBox().x - 9,
          domPart.getBBox().y - 9,
          domPart.getBBox().w + 18,
          domPart.getBBox().h + 18
        )
        highlighter.box.node.id = "highlighter-rect"
